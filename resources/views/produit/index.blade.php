...
<div class="container mt-5">
  <div class="row">
      ...
      <form action="{{ route('produit') }}" method="GET" class="form-inline">
        @csrf
        <div class="input-group mb-3">
          ...
          <a class="btn btn-outline-success" href="{{ url('/produit/create') }}">
            {{-- Add --}}
      ...
      <tbody>
        ...
        @foreach ($produit as $p)
            <tr>
              <td>{{$p->id}}</td>
              <td>
                <img src="{{asset('storage/'.$p->photo)}}" width="150" class="img-thumb" alt="">
              </td>
              <td>{{$p->libelle}}</td>
              <td>{{$p->prix}}</td>
              <td>
                  
                <a href="{{ url('produit/'.$p->id.'/show') }}" class="btn btn-outline-info btn-sm ">
                ...
                <a href="{{ url('produit/'.$p->id.'/edit') }}" class="btn btn-warning btn-sm">
                  ...
                  <form action="{{url('produit/'.$p->id.'/delete')}}" method="post" class="d-inline">
                      @csrf
                      @method('DELETE')
